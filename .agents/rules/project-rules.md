...
Before responding to any user request, formulating a plan, or analyzing any code, you must independently load and read the current contents of `@docs/problem.md`, `@docs/problem-vision-gaps.md`, `@docs/problem-vision-wip.md`, `@docs/vision.md`, `@docs/architecture.md`, `@docs/specifications.md`, `@docs/vision-architecture-gaps.md`, `@docs/architecture-specifications-gaps.md`, `@docs/vision-architecture-wip.md`, and `@docs/architecture-specifications-wip.md`. All proposed solutions and tests must be strictly evaluated against the definitions in these documents.
...
1. You must cross-reference the Problem against the Vision to ensure every stated failure the system aims to solve has a corresponding goal or tenet in the Vision.
2. You must cross-reference the Vision against the Architecture to ensure every overarching goal, constraint, and requirement actually has an architectural mechanism actively designed to fulfill it. 
3. You must cross-reference the Architecture against the Specifications to ensure every abstract architectural component has concrete, testable behaviors and deterministic error states defined.
...
1. **Problem:** Decide if a change to `@docs/problem.md` (or its associated gaps/WIPs in `@docs/problem-vision-gaps.md` and `@docs/problem-vision-wip.md`) is required. If so, present and iterate per the modification protocol below.
2. **Vision:** Decide if a change to `@docs/vision.md` (or its associated gaps/WIPs in `@docs/vision-architecture-gaps.md` and `@docs/vision-architecture-wip.md`) is required. If so, present and iterate per the modification protocol below.
3. **Architecture:** Decide if a change to `@docs/architecture.md` (or its associated gaps/WIPs in `@docs/architecture-specifications-gaps.md` and `@docs/architecture-specifications-wip.md`) is required. If so, present and iterate per the modification protocol below.
4. **Specifications:** Decide if a change to `@docs/specifications.md` is required (after thorough review of the vision, architecture, and specifications). If so, present and iterate per the modification protocol below.
5. **Tests & Implementation:** Decide if a change to the tests is needed (after reviewing all documentation levels above). Because tests strictly map to active documentation, simply execute the tests followed by the runtime code without requiring textual iteration. When running the test suite, always run the complete suite with no exclusions — including live API and end-to-end tests. Never filter out or skip tests when verifying correctness. You must always run tests in verbose output mode and pipe the results to a `test_log.txt` file in the `logs/` folder (overwriting the file on each run, do not append). You must then read this file to see the results. For end-to-end tests, you should use the built-in debug logging mechanism of the framework to do tracing. If specific logging lines were added while debugging a breaking test, these should be removed once the test has been fixed, unless you believe the logging line will be helpful in the future for the general case.