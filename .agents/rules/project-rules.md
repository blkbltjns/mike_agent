...
5. **Tests & Implementation:** Decide if a change to the tests is needed (after reviewing all documentation levels above). Because tests strictly map to active documentation, simply execute the tests followed by the runtime code without requiring textual iteration. Please refer to the `testing` SKILL for all specific checking, linting, and test execution procedures.