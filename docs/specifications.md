...
* **`process_user_prompt` Execution:** The LLMAgent must treat every `process_user_prompt` command as a completely isolated, context-free execution. It executes in two phases. Phase 1: The user's prompt is sent to the LLM with an instruction to respond in natural text describing what context it needs before answering. The LLMAgent enqueues a `gather_context` command with that natural text as its payload and registers the command ID in `waiting_for_results`. Phase 2: When the `gather_context` result arrives in the Outbox, the LLMAgent prepends the accumulated context to the original prompt and makes a second LLM call. The Phase 2 response is written directly to the Outbox as the final result of the `process_user_prompt` command with no additional formatting imposed by the agent.
