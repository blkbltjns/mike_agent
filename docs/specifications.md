...
* **System Bootstrapping Validation:** The system entrypoint must inject an initial `prompt_user` command into the LLMAgent's `run(bootstrap_commands)` method. This bootstrapped command must be deterministically tracked so the agent's `handle_outbox_result()` naturally captures the human's first response based solely on the command type returned.
