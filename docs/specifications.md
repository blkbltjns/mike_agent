...
* **System Initialization:** Base Agents must accept an optional `bootstrap_commands` argument (a list of `AgentCommand` objects) in their `run` method. Prior to entering the polling loop, the agent must submit these commands to the Bus via `broadcast_to_one` and register the command ID in its internal `waiting_for_results` state tracker.
...
* **Proactive & Reactive Capabilities:** The REPL must provide commands allowing the human to proactively `enqueue` new target commands, toggle system observables (the terminal inputs `debug on` and `debug off` must automatically enqueue a corresponding `toggle_debug_logging` broadcast), as well as reactively `reply` to matching commands waiting in the Inbox.
...
* **Live Dialogue Routing:** The `UserAgent` exclusively shepherds the auto mode dialogue state. It issues proactive `process_user_prompt` commands containing the human's text directly to the Message Bus and tracks their Outbox completions. The `LLMAgent` acts purely as an asynchronous executor—it claims `process_user_prompt` commands, coordinates its Phase 1 and 2 operations, and writes the final string to the Outbox. For auto mode conversations, the `LLMAgent` no longer tracks these commands or synthesizes reciprocal `prompt_user` commands in response.
